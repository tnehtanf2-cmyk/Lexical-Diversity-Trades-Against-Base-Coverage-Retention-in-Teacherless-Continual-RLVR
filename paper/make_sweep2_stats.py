...
Output: paper/results_sweep2_stats.json
"""
...
for c in SWEEP_CONDS:
    ...
    for s in SEEDS:
        ...
        per["cov"].append(covret_mean(f"{LOG}/sweep2_{c}_s{s}_meta.jsonl", base_sweep,
                                      set(STEADY_SWEEP)))
    sweep[c] = per
...
with open(f"{HERE}/results_sweep2_stats.json", "w") as f:
    json.dump(out, f, indent=1)

print("OK: results_sweep2_stats.json (full precision, exact block permutation)")
